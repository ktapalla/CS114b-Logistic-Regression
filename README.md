...
\end{bmatrix} 
= 
\sum\limits_{j=1}^{n} *x*~j~ *w*~j~ + 1 \times *b* =  **x** \cdot **w** + b 
``` 
...
**x** = 
\begin{bmatrix}
*x*_{1}^{(1)} \cdots *x*_{n}^{(1)} 1\\
\vdots \ddots \vdots \vdots \\
*x*_{1}^{(*m*)} \cdots *x*_{n}^{(*m*)} 1\\
\end{bmatrix} 
**y** = 
\begin{bmatrix}
*y*^{(1)} \vdots *y*^{(1)} \\
...
3. Update the cross-entropy loss. Recall that the loss (for a single example) is $**L**_{CE}(\hat{y}, y) = -[*y*log\hat{y} + (1-*y*)log(1-\hat{y})]$. We want to calculate the average train loss over the whole training set, but since we're dividing by the number of training documents at the end, it's okay to just keep a running sum. 
